...
def rank_card(username, avatar, level, rank, current_xp, custom_background, xp_color, next_level_xp):
    # create backdrop
    img = Image.new('RGBA', (934, 282), color=(255, 255, 255, 0))
    ...
    d = drawProgressBar(d, 260, 180, 575, 40, current_xp / next_level_xp, bg="#484B4E",
                             fg=xp_color)  # create progress bar
    print(str(os.getcwd()))

    font = ImageFont.truetype(font=f"regular.ttf", size=50)
    font2 = ImageFont.truetype(font=f"regular.ttf", size=25)
    ...
    # save image
    img.save(f"rankcard.png")
    return f"rankcard.png"
